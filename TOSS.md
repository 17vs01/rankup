...
- 홈 랭킹 카드에 내 주간 점수와 리셋까지 남은 시간이 뜬다

이유: 리더보드가 하나뿐인데 평생 누적을 올리면 상위권이 고착돼 새로 온 사람이 첫날 포기한다.
주간 리셋은 매주 모두를 같은 출발선에 세운다 (Duolingo 리그와 같은 이유).

`js/platform.js`의 `compositeScore()`(전 종목 평균 기반 종합 점수)는 남겨 뒀다.
평생 누적 기준으로 되돌리려면 `endSession`의 `submitScore(weeklyScore())`만 바꾸면 된다.
