...
function genTournament() {
  ...
  const n = ri(20, 90);
  // 반씩 접어보면 어디서 홀수가 나오는지 — 부전승을 따로 챙겨야 하는 지점
  const chain = [n];
  let x = n;
  while (x > 1 && x % 2 === 0 && chain.length < 6) { x /= 2; chain.push(x); }
  const hard = n % 2 === 1
    ? `첫 라운드부터 ${n}팀이 홀수라 부전승을 챙겨야 합니다`
    : (x % 2 === 1 && x > 1
      ? `${chain.join(' → ')} 로 접다 보면 ${x}에서 홀수가 나와 부전승을 챙겨야 합니다`
      : `${chain.join(' → ')} … 라운드마다 일일이 세야 합니다`);
  return {
    q: `${n}개 팀이 <b>토너먼트</b>로 우승팀을 가립니다. 한 번이라도 지면 바로 탈락하고, 팀 수가 딱 맞지 않는 라운드에서는 몇몇 팀이 <b>부전승</b>(경기 없이 다음 라운드로)을 받습니다. 우승팀이 나올 때까지 총 몇 경기가 필요할까요?`,
    ans: n - 1, trap: Math.round(n / 2),
    alts: [n, Math.round(n / 2) + Math.round(n / 4), n * 2], unit: '경기',
    why: `라운드별로 세면 ${hard}. 뒤집어 보세요. 대회가 끝나면 우승팀 하나만 남고 <b>나머지 ${n - 1}팀은 전부 탈락</b>합니다. 그런데 탈락은 오직 "경기에서 졌을 때"만 일어나고, 한 경기에서 지는 팀은 <b>정확히 하나</b>입니다. 부전승은 경기가 아니라서 아무도 떨어뜨리지 않고요. 그러니 필요한 경기 수 = 탈락한 팀 수 = ${n - 1}경기입니다. 대진표를 어떻게 짜든 답은 같습니다.`,
  };
}
